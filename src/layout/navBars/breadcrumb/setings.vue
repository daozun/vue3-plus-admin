...
import { getLightColor, verifyAndSpace, wartermark as Watermark } from 'base-core-lib-ts';
import { Local } from '@/utils/storage';
import commonFunction from '@/hooks/commonFunction';
export default defineComponent({
	...
	setup() {
		...
		const onClassicSplitMenuChange = () => {
			...
			setLocalThemeConfig();
			proxy.EventBus.emit('getBreadcrumbIndexSetFilterRoutes');
		};
		// 4、界面显示 --> 侧边栏 Logo
		...
		const onSortableTagsViewChange = () => {
			proxy.EventBus.emit('openOrCloseSortable');
			setLocalThemeConfig();
		};
		// 4、界面显示 --> 开启 TagsView 共用
		const onShareTagsViewChange = () => {
			proxy.EventBus.emit('openShareTagsView');
			setLocalThemeConfig();
		};
		// 4、界面显示 --> 灰色模式/色弱模式
		...
		onMounted(() => {
			nextTick(() => {
				...
				// 修复防止退出登录再进入界面时，需要刷新样式才生效的问题，初始化布局样式等(登录的时候触发，目前方案)
				proxy.EventBus.on('onSignInClick', () => {
					initSetStyle();
				});
				// 监听菜单点击，菜单字体背景高亮
				proxy.EventBus.on('onMenuClick', () => {
					onMenuBarHighlightChange();
				});
				// 监听窗口大小改变，非默认布局，设置成默认布局（适配移动端）
				proxy.EventBus.on('layoutMobileResize', (res: any) => {
		...
		onUnmounted(() => {
			// 取消监听菜单点击，菜单字体背景高亮
			proxy.EventBus.off('onMenuClick');
			proxy.EventBus.off('onSignInClick');
			proxy.EventBus.off('layoutMobileResize');
		});
		return {
			openDrawer,
			onColorPickerChange,
