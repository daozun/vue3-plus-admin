	...
	setup() {
		...
		const onScreenfullClick = () => {
			...
			screenfull.on('change', () => {
				if ((screenfull as any).isFullscreen) state.isScreenfull = true;
				else state.isScreenfull = false;
			});
		};
		// 布局配置 icon 点击时
		const onLayoutSetingClick = () => {
			proxy.EventBus.emit('openSetingsDrawer');
		};
		// 下拉菜单点击时
		...
		const setI18nConfig = (locale: string) => {
			proxy.EventBus.emit('getI18nConfig', proxy.$i18n.messages[locale]);
		};
		// 初始化言语国际化
		const initI18n = () => {
