...
import { isObjectValueEqual } from 'base-core-lib-ts';
...
export default {
	...
	setup() {
		...
		const addTagsView = (path: string, to?: any) => {
			...
			nextTick(async () => {
				...
				if (to && to.meta.isDynamic) {
					...
					item = state.tagsViewRoutesList.find((v: any) => v.path === path) as any;
				}
		...
		const refreshCurrentTagsView = (fullPath: string) => {
			proxy.EventBus.emit('onTagsViewRefreshRouterView', fullPath);
		};
			...
			return itemRoute.find((v: any) => {
				if (
					...
					isObjectValueEqual(
						...
						(cParams && Object.keys(cParams ? cParams : {}).length > 0 ? cParams : null) as any
					)
				) {
					return v;
		...
		// 拖动问题，小于 1000px 时自动设置禁止拖拽。鼠标的滚轮方向是反的，往下滑是向左在手机上，只能通过滑动来滚动，但是点击会触发标签排序。
		...
		onBeforeMount(() => {
			...
			proxy.EventBus.on('onCurrentContextmenuClick', (data: object) => {
				onCurrentContextmenuClick(data);
			});
			// 监听布局配置界面开启/关闭拖拽
			proxy.EventBus.on('openOrCloseSortable', () => {
				initSortable();
			});
			// 监听布局配置开启 TagsView 共用，为了演示还原默认值
			proxy.EventBus.on('openShareTagsView', () => {
				if (getThemeConfig.value.isShareTagsView) {
					router.push('/home');
					state.tagsViewList = [];
					state.tagsViewRoutesList.map((v: any) => {
		...
		onUnmounted(() => {
			// 取消非本页面调用监听
			proxy.EventBus.off('onCurrentContextmenuClick');
			// 取消监听布局配置界面开启/关闭拖拽
			proxy.EventBus.off('openOrCloseSortable');
			// 取消监听布局配置开启 TagsView 共用
			proxy.EventBus.off('openShareTagsView');
			// 取消窗口 resize 监听
