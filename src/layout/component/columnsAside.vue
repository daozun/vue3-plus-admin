...
export default {
	...
	setup() {
		...
		const onColumnsAsideMenuMouseenter = (v: Object, k: number) => {
			...
			proxy.EventBus.emit('setSendColumnsChildren', setSendChildren(path));
		...
		const onColumnsAsideMenuMouseleave = async () => {
			...
			setTimeout(() => {
				...
				if (!isColumnsMenuHover && !isColumnsNavHover) proxy.EventBus.emit('restoreDefault');
			}, 100);
		...
		const setFilterRoutes = () => {
			...
			proxy.EventBus.emit('setSendColumnsChildren', resData);
		};
		// 传送当前子级数据到菜单中
		...
		watch(store.state, (val) => {
			...
			if (!val.routesList.isColumnsMenuHover && !val.routesList.isColumnsNavHover) {
				...
				proxy.EventBus.emit('setSendColumnsChildren', setSendChildren(route.path));
			} else {
				state.liHoverIndex = state.liOldIndex;
				if (!state.liOldPath) return false;
				proxy.EventBus.emit('setSendColumnsChildren', setSendChildren(state.liOldPath));
			}
		});
		// 页面加载时
		onMounted(() => {
			...
			proxy.EventBus.on('restoreDefault', () => {
		...
		// 页面卸载时
		onUnmounted(() => {
			proxy.EventBus.off('restoreDefault', () => {});
		});
		// 路由更新时
		onBeforeRouteUpdate((to) => {
			setColumnsMenuHighlight(to.path);
			proxy.EventBus.emit('setSendColumnsChildren', setSendChildren(to.path));
		});
		return {
