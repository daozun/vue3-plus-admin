...
export default {
	...
	setup() {
		...
		const onAsideEnterLeave = (bool: Boolean) => {
			...
			if (!bool) proxy.EventBus.emit('restoreDefault');
		...
		onBeforeMount(() => {
			...
			setFilterRoutes();
			// 此界面不需要取消监听(proxy.EventBus.off('setSendColumnsChildren))
			// 因为切换布局时有的监听需要使用，取消了监听，某些操作将不生效
			proxy.EventBus.on('setSendColumnsChildren', (res: any) => {
				state.menuList = res.children;
			});
			proxy.EventBus.on('setSendClassicChildren', (res: any) => {
			...
			proxy.EventBus.on('getBreadcrumbIndexSetFilterRoutes', () => {
				setFilterRoutes();
			});
