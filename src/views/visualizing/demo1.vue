			...
			<div class="visualizing-container-head">
				...
				<div class="visualizing-container-head-center">
					<div class="visualizing-container-head-center-box">
						<div class="visualizing-container-head-center-maintitle">xxx软件科技有限公司</div>
...
export default defineComponent({
	...
	setup() {
		...
		// 初始化时间
		const initTime = () => {
			state.time.txt = dayjs().format('YYYY-MM-DD HH:mm:ss');
		...
		const initEchartsMap = () => {
			const myChart = echarts.init(document.getElementById('visualizingDemo1') as any) as any;
			const option = {
				tooltip: {
					trigger: 'item',
					formatter(params) {
			...
			const { BMap, BMAP_SATELLITE_MAP } = window as any;

			map.setMapType(BMAP_SATELLITE_MAP);

			let bdary = new BMap.Boundary();
			// 获取行政区域
			bdary.get('辽宁', function (rs) {
				// 行政区域的点有多少个
				...
				new BMap.Point('大连市', 8);

				map.centerAndZoom(new BMap.Point(121.561836, 38.984469), 8);
			});
		};
		// 产业概况
...
.visualizing-demo1 {
	...
	.visualizing-container {
		...
		.visualizing-container-title-colorful {
			--mask-image: repeating-linear-gradient(135deg, #000 0, #000 1px, transparent 1px, transparent 4px);
...
function BMAP_SATELLITE_MAP(BMAP_SATELLITE_MAP: any) { throw new Error('Function not implemented.'); } function BMAP_SATELLITE_MAP(BMAP_SATELLITE_MAP:
